...
int main(int argc, char **argv) {
  ...
  if (rank == 0) {
    ...
    MPI_Send(&local_value, 1, MPI_INT, 1, MPI_ANY_TAG, MPI_COMM_WORLD);
    // 2- Receive the value from process 1 (in other_value)
    MPI_Recv(&other_value, 1, MPI_INT, 1, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    MPI_Recv(&other_value, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    // 2- Send the value to process 0
    MPI_Send(&other_value, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD);
