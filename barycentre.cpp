void compute(int total_count, int my_count, float my_points[][3]) {
  ...
  for (int i=0; i < my_count; ++i) {
    ...
    float x = my_points[i][0] - barycentre[0];
    float y = my_points[i][1] - barycentre[1];
    float z = my_points[i][2] - barycentre[2];
    float dista = std::sqrt(x * x + y * y + z * z);
    // And printing the result
    std::cout << rank << " " << dista << std::endl;
  }
}